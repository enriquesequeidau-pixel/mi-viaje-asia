...
try {
  ...
  const { data: signed, error: signedError } = await userClient.storage.from('trip-media').createSignedUrl(storagePath, 60);
  ...
  console.log(JSON.stringify({ upload: true, signedRead: true, delete: true }));
} finally {
